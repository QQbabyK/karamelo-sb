...
class Universe : protected Pointers {
 public:
  ...
  vector<std::array<int, 3>> sendnrecv;  ///< stores the send and receive pattern to follow. The vector's length corresponds to the number of steps. For a given step, there is an array of two ints, the first int is either 0 (receive) or 1 (send), the second it the rank of the CPU to receive or send to.
